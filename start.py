...
class queueController:
	...
	def push_qProcessing(self):
		...
		funcionBat1 = funcionBat(self.pathData, self.threadPE, self.mainGUI)
		future = self.threadPE.submit(funcionBat1.do_bat, nowData)
...
class mainGUI(tk.Frame):
	...
	def makeRow(self, type, text, row, default, defaultOp):
	...
	def create_widgets(self):
		...
		self.input_seed = self.makeRow("basic", "seed", 2, "75", "defaultOp")
		self.input_n_iter = self.makeRow("basic", "n_iter", 3, "1", "defaultOp")
		self.input_n_samples = self.makeRow("basic", "n_samples", 4, "5", "defaultOp")
		self.input_H, self.input_H_op1, self.input_H_op2 = self.makeRow("optional", "H", 5, "640", "64")
		self.input_W, self.input_W_op1, self.input_W_op2 = self.makeRow("optional", "W", 6, "512", "64")
		self.input_ddim_steps, self.input_ddim_steps_op1, self.input_ddim_steps_op2 = self.makeRow("optional", "ddim_steps", 7, "50", "10")
		self.input_init_img = self.makeRow("long", "init_img", 8, "", "defaultOp")
		self.input_strength, self.input_strength_op1, self.input_strength_op2 = self.makeRow("optional", "strength", 9, "0.5", "0.1")
		self.input_tileable = self.makeRow("basic", "tileable", 10, "", "defaultOp")
		self.input_outdir = self.makeRow("long", "outdir", 11, "D:\Desktop\AIImage\do_output", "defaultOp")
		...
		self.run_button2 = tk.Button(self, text="Output", width=20, command=lambda: do_output(myInputController, myQueueController, myFileController))
		...
		self.run_button3 = tk.Button(self, text="Kill a waiting task", width=20, command="")
...
def do_output(inputController, queueController, fileController):

	myOrderData = inputController.InputToOrder()
	fileController.write_log()
	queueController.push_qWaiting(myOrderData)
...
myInputController.deleteInput()
myInputController.orderToInput()
